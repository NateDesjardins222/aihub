...
| G4 | **Authoritative product model** | **READY FOR TEST** | Phase 3: one authoritative model; DB=catalog on all fields; normal seed produces the 10; legacy retired; fresh+existing DB verified; integrity tests guard divergence. Phase 3.5: risk/payout semantics LOCKED (EOD floor locks at start, `trailingLockAtMicros=0`; breach on equity; CORE/SELECT $0 buffer; post-payout floor safe). Phase 8: ALL 10 products proven against cross-contamination — config matrix (153 tests) + runtime provisioning matrix (each account provisions with its OWN pinned terms, no 50K leak, family floors/buffers/consistency distinct) + payout engine reads per-account pinned rules (`PRODUCT_MATRIX_ACCEPTANCE.md`) | ~~HTF-5, HTF-6, DR-1, DR-2~~ resolved; ~~DR-11~~ resolved (Phase 3.5); owner sign-off on values still recommended |
...
| G6 | **Security / trust boundaries** | **PARTIAL (hardened)** | Strong server-authority design; Phase 4 closed the fail-open providers (~~HTF-1, HTF-2~~) and gated `/design-lab` + dev seed (~~HTF-4~~); Phase 7 gated the trader self-serve rule/reset/environment routes to PRACTICE-only (~~HTF-21~~) and made the money/lifecycle kill switches actually enforce (~~HTF-24~~). **Phase 10:** adversarial hardening — enforced boundaries documented (`SECURITY_MODEL.md`, `THREAT_MODEL.md`); secret scan clean (no `.env`/Rithmic password committed; 0 server secrets in the web bundle); dev/mock routes verified prod-gated; audit integrity proven at scale (`audit-chain-stress`); adversarial suites green in isolation; `pnpm audit` prod-clean (one moderate DEV-ONLY esbuild advisory via drizzle-kit, HTF-25). Remaining: console surfacing of the safety mutations (HTF-10). Held at PARTIAL — never VERIFIED from code per the mandate; no third-party pentest | HTF-10 (console surfacing); HTF-25 (dev-only) |
...
| G11 | **Observability / alerting** | **PARTIAL (improved)** | Audit chain strong. **Phase 11:** liveness (`/health`) now distinct from **readiness** (`/ready`, DB-probing 503, no fake green); **release identity** (`/version`, `release` field); Owner System Doctor surfaces DB/provider/reconciliation/jobs health; logging/metrics/alert model documented (`OBSERVABILITY.md`). Remaining: metrics/tracing export endpoint; console buttons for safety mutations | HTF-10, HTF-19 |
| G12 | **Infrastructure / deploy / backups** | **PARTIAL** | **Phase 11:** backup→drop→restore **proven** on an isolated DB with $0 reconciliation delta + audit re-verify (`RECOVERY_DRILL_REPORT.md`); restart/crash/outage safety proven (engine reconstructs from Postgres; kill switches durable; graceful shutdown); DR/deploy/release/incident runbooks written; canonical `pnpm validate:release`; HTF-22 fixed. Remaining (external gates): automated scheduled backups + WAL/PITR, prod hosting + deploy manifests, inactivity cron binding (HTF-18), CI | HTF-16, HTF-17, HTF-18 |

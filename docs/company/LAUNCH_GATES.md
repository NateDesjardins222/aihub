...
| G6 | **Security / trust boundaries** | **PARTIAL** | Strong server-authority design; Phase 4 closed the fail-open providers (~~HTF-1, HTF-2~~) and gated `/design-lab` + dev seed (~~HTF-4~~); Phase 7 gated the trader self-serve rule/reset/environment routes to PRACTICE-only (~~HTF-21~~) and made the money/lifecycle kill switches actually enforce (~~HTF-24~~); remaining: console surfacing of the safety mutations | HTF-10 (console surfacing) |
